...
$query_rs_keyED = "SELECT un_LlaveInicial, un_LlaveIv, un_MetodoEncriptacion, un_TipoHash FROM un_control WHERE un_IdEstado = 1;" ;
$rs_keyED = mysqli_query($cnn_kn, $query_rs_keyED) or die(mysqli_error()."Err.....$query_rs_key");
...
if ($resultado = mysqli_query($cnn_kn, $query_rs_keyED)) {
	...
	{ 
		$secret_key = trim($strowreg["un_LlaveInicial"]);
        $secret_iv = trim($strowreg["un_LlaveIv"]);
        $encrypt_method = trim($strowreg["un_MetodoEncriptacion"]);
        $tipo_hash = trim($strowreg["un_TipoHash"]);		
	}
}
