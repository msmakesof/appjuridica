...
if (isset($_POST['from'])) 
{
    ...
    if ($_POST['from']!="" AND $_POST['to']!="") 
    {
        ...
        //$fi = " STR_TO_DATE('".$inicio_normal."','%Y-%m-%d')";
        //$fi = strtotime($inicio_normal);
        $fi = date('Y-m-d',strtotime($inicio_normal));


        // y la formateamos con la funcion _formatear
        $final_normal  = $_POST['to'];
        //$ff = " STR_TO_DATE('".$final_normal."','%Y-%m-%d')";
         $ff = date('Y-m-d',strtotime($final_normal));
        ...
        {    
            ...
            mysqli_select_db($cnn_kn, $database_cnn_kn);
            $query_rs_cusuario = "SELECT count(IdClase) as totclases FROM clases WHERE Sede = $sede AND Salon = $salon AND Materia = $materia AND Nivel = $nivel AND Horario = $horario AND Profesor = $profesor AND desde = '$fi' AND hasta = '$ff' ;" ;
            $rs_cusuario = mysqli_query($cnn_kn, $query_rs_cusuario) or die(mysqli_error()."Err.....$query_rs_usuario");
            $row_rs_cusuario = mysqli_fetch_assoc($rs_cusuario);
            $totalRows_rs_cusuario = mysqli_num_rows($rs_cusuario);
            echo "sel...$query_rs_cusuario<br>";
             ?>
            <script type="text/javascript">    
            $(document).ready(function(){
                alert("<?php echo $query_rs_cusuario; ?>");
                console.log(<?php echo $query_rs_cusuario; ?>);
                //alert(x)
            });   
            </script>
            <?php

            if ($row_rs_cusuario['totclases'] == 0)
            {                

                $insertSQL ="";
                //$insertSQL = "INSERT INTO eventos VALUES(null,'$titulo','$body','','$clase','$inicio','$final','$inicio_normal','$final_normal')";
                $insertSQL = "INSERT INTO eventos VALUES(null,'$titulo','$body','', '$clase', '$inicio', '$final', '$fi', '$ff')";
                mysqli_select_db($cnn_kn, $database_cnn_kn);
                echo "insEve....$insertSQL<br>";
                ...
                { 
                    $insertSQL = "";
                    $diaAdd = $dias[$i];
                    $insertSQL = "INSERT INTO clases (Sede, Salon, Materia, Nivel, Horario, Profesor, Estado, dia, desde, hasta, FechaGraba, IdEvento) VALUES ($sede, $salon, $materia, $nivel, $horario, $profesor, 1, '$diaAdd', '$fi', '$ff', Now(), $id);";
                    //echo "insert......$insertSQL<br>";  
                    echo "insCla....$insertSQL<br>";  
                    mysqli_select_db($cnn_kn, $database_cnn_kn);
                    $Result1 = mysqli_query($cnn_kn, $insertSQL) or die(mysqli_error()."Err....$insertSQL<br>");
                }
                ...
                mysqli_free_result($rs_cusuario);
            ...
            $(document).ready(function(){
                swal({
                  title: "Atención:  Esta Programación ya Existe.",
                        ...
                        onAfterEventsLoad: function(events)
                        {
                                ...
                                $.each(events, function(key, val)
                                {
                                        $(document.createElement('li'))
                                                .html('<a href="' + val.url + '">' + val.title + ' '+ val.nombreProfesor +'</a>')
                                                .appendTo(list);
                                });
                        },
