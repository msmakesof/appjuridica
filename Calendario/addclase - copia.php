...
//echo $horario.'+'.$sede.'+'.$profesor.'+'.$salon.'+'.$desde.'+'.$hasta.'+'.$materia.'+'.$nivel.'<br>';
//$array[5]
...
{  
    ...
    if( $reg == 0)
    {
       mysqli_select_db($cnn_kn, $database_cnn_kn);
        ...
        $insertSQL = "";        
        $insertSQL = "INSERT INTO clases (Sede, Salon, Materia, Nivel, Horario, Profesor, Estado, dia, desde, hasta, FechaGraba, IdEvento) VALUES ($sede, $salon, $materia, $nivel, $horario, $profesor, 1, '', '$fechaini', '$fechafin', Now(), 0);";          
        mysqli_select_db($cnn_kn, $database_cnn_kn);
        ...
        { 
          ...
          mysqli_select_db($cnn_kn, $database_cnn_kn);
          $Result1 = mysqli_query($cnn_kn, $insertSQL) or die(mysqli_error()."Err10....$insertSQL<br>"); 
        }  
