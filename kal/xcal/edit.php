...
<html>
    ...
    <script type="text/javascript">  
        ...
        $(document).ready(function() {
			...
			$("#Savebtn").on("click", function(e){
                ...
                var start = "<?php echo $start ;?>"; //$("#hf_start").val();
                if( !sede || !profesor || !salon || !horario || !nivel || !materia )
                {                    
                    swal({
                      title: "Atención:  Debe seleccionar informacion en todos los campos...",
                      text: "un momento por favor.",
                ...
                {                 
                    ...
                    //
                    //alert('<?php echo isset($event); ?>');
                    var x = "<?php echo isset($event); ?>";
                    var y = "<?php echo $IdjqCal; ?>";
                    // php/datafeedgea.php?method=adddetails<?php echo isset($event); ?>&id=<?php echo $IdjqCal; ?>    
                    $.ajax({
                        //this is the php file that processes the data 
                        url: "addCalendar.php",
                        //GET method is used
                        type: "POST",
                        //data: "method=adddetails&id="+y+"&sede="+sede+"&profesor="+profesor+"&salon="+salon+"&horario="+horario+"&nivel="+nivel+"&materia="+materia+"&hf_start="+start, 
                        data: "id="+y+"&sede="+sede+"&profesor="+profesor+"&salon="+salon+"&horario="+horario+"&nivel="+nivel+"&materia="+materia+"&hf_start="+start, 
                        beforeSend: function() {
                          $("#msj").html("<img src='../../sweet/47.gif'>");
                        },
                        success: function (xdata) {
                            ...
                            if(xdata == "R")
                            {
                                //$("#msj").html(data);
                                swal({
                                  title: "Atención:  Registro ya Existe.",
                                  text: "un momento por favor.",
                                  imageUrl: "../../sweet/alert.png",
                                  timer: 2000,
                                  showConfirmButton: false
                            ...
                            if(xdata == "N")
                            {
                                //$("#msj").html(data);
                                swal({
                                  title: "Atención:  Registro No se pudo Grabar.",
                                  text: "un momento por favor.",
                                  imageUrl: "../../sweet/2.gif",
                                  timer: 2000,
                                  showConfirmButton: false
          ...
          <label>                    
            ...
            <div class="form-group">                        
                <div class="input-group date form_date col-md-5" data-date="" data-date-format="dd MM yyyy" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
                    <input class="form-control" size="16" type="text" value="<?php echo $start; ?>" id="from" name="from" readonly >
            ...
            <div class="form-group">
                ...
                <div class="input-group date form_date col-md-5" data-date="" data-date-format="dd MM yyyy" data-link-field="dtp_input2" data-link-format="yyyy-mm-dd">
                    <input class="form-control" size="16" type="text" id="to" name="to" readonly value="<?php echo $start; ?>">
                    <span class="input-group-addon"><span class="glyphicon glyphicon-remove"></span></span>
                    <span class="input-group-addon"><span class="glyphicon glyphicon-calendar"></span></span>
                </div>
 ...
 -->
          ...
          <input id="hf_start" name="hf_start" type="hidden" value="<?php echo $start; ?>" />          
        </form>                 
      </div>
      <div id="msj"></div>
    </div>
