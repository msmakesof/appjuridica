...
mysql_select_db($database_cnn_plan_mejora, $cnn_plan_mejora);
$query_rs_privilegios = "SELECT nombreEmp, cod_suc, control, graba, modifica, consulta, borra, racolp, admin, muestra_botones FROM empleados WHERE usuario = '$colname_rs_privilegios'";
...
$muestra_btn_usr = $row_rs_privilegios["muestra_botones"];
...
mysql_select_db($database_cnn_plan_mejora, $cnn_plan_mejora);
$query_rs_jc = "select jqcalendar.*, temas.nombre as nombretema from jqcalendar join tipo_auditoria on tipo_auditoria.cod_ti_audit = jqcalendar.tipo_auditoria join auditoria on auditoria.id = jqcalendar.auditoria join listachqxtipoaudit on listachqxtipoaudit.id_listacheq = jqcalendar.listachq left join temas on temas.id = jqcalendar.tema AND temas.estado = 'A' where jqcalendar.id = ".$id;
...
$dirigido_a =$row_rs_jc['dirigido_a'];
$tipolista =$row_rs_jc['tipolista'];
$Location =$row_rs_jc['Location'];        // crono
$responsable =$row_rs_jc['responsable'];  // comite
...
function grabar(formName,action){			
		...
		{
			alert("ATENCION: Debe seleccionar informacion en el campo Detalle de la Actividad.");
			return;
		}
...
<div id="centrardiv">
      ...
      <input name="responsable" type="text" class="span4" id="responsable" style="width:55%; height:28px" value="<?php echo $row_rs_jc["responsable"]; ?>" MaxLength="200" <?php if($isadmin != "Y") {?> readonly <?php } ?>/>
		                     
      <input id="colorvalue" name="colorvalue" type="hidden" value="<?php echo $row_rs_jc["Color"]; ?>" <?php if($isadmin != "Y") {?>  readonly="readonly" <?php } ?>/>                
          </label>
					
					<label>                    Desde
					 <input MaxLength="10" class="required date" id="stpartdate" name="stpartdate" style="padding-left:2px;width:88px; height:28px" type="text" value="<?php echo $row_rs_jc["StartTime"]; ?>" <?php if($isadmin != "Y") {?>  readonly="readonly" <?php } ?>/>                       
Hasta                       
              <input MaxLength="10" class="required date" id="etpartdate" name="etpartdate" style="padding-left:2px;width:88px; height:28px" type="text" value="<?php echo $row_rs_jc["EndTime"]; ?>" <?php if($isadmin != "Y") {?>  readonly="readonly" <?php } ?>/>               </label>
							
					
					<label>                    
            <span>                        Proceso:
            </span>                    
            <input MaxLength="200" id="Location" name="Location" style="width:65%; height:28px" type="text" value="<?php echo $row_rs_jc["Location"]; ?>" <?php if($isadmin != "Y") {?>  readonly="readonly"  <?php } ?>/>                 
          </label>  
...
<?php echo $row_rs_jc["actividad"]; ?>" <?php if($isadmin != "Y") {?> readonly <?php } ?> />                
          </label> 
...
<?php echo $row_rs_jc["observacion"]; ?>
					...
					if($tipolista == 1)
					{						
							if(strtoupper($racolss) == $responsable ) {$muestra_btn = "V";} 	
					}
					// Capacitacion
					if($tipolista == 2){
						  if(strtoupper($racolss) == $dirigido_a ) {$muestra_btn = "V";} 	
					}	
					// Comite
					if($tipolista == 3){
							if(strtoupper($racolss) == $responsable ) {$muestra_btn = "V";} 	
					...
					<?php 
					if($muestra_btn == "V" || $muestra_btn_usr == "V"){ ?>
							<a href="#" onClick="grabar('form1','submit');" class="btn btn-primary"> Grabar </a>&nbsp;&nbsp;&nbsp;&nbsp;
					<?php } ?>
					
					<?php 
					if(strtoupper($racolss) != "GCI" ) {?>
					<a href="../../gestionn.php" class="btn btn-danger"> Salir </a>
					<?php } else {?>
					<a href="../../sub_index.php" class="btn btn-danger"> Salir </a>
					<?php } ?>
					</label> 
					
					<label>  
					<?php if($muestra_btn == "V") { ?>                  
