...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $ta, $au, $lc, $color, $observacion, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "insert into jqcalendar (subject, starttime, endtime, isalldayevent, description, location, tipo_auditoria, auditoria, listachq, color, observacion) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
      ...
      .mysql_real_escape_string($ta)."', '"
			...
			."'13','".mysql_real_escape_string($observacion)."')";
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $ta, $au, $lc, $color, $observacion, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    ...
    case "adddetails":
        ...
        if(isset($_GET["id"])){
            $ret = updateDetailedCalendar($_GET["id"], $st, $et, 
                ...
                $_POST["Location"], $_POST["ta"], $_POST["au"], $_POST["lc"], $_POST["colorvalue"], $_POST["observacion"], $_POST["timezone"]);
        }else{
