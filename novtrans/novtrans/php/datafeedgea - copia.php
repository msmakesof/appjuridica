...
function addDetailedCalendar($st, $et, $sub, $ade, $dscr, $loc, $ta, $au, $lc, $color, $observacion, $actividad, $tz, $medio){
  ...
  try{
    ...
    $db->getConnection();
		...
		//
    $sql = "insert into jqcalendar (responsable, starttime, endtime, isalldayevent, description, location, tipo_auditoria, auditoria, listachq, color, observacion, actividad, medio) values ('"
      .mysql_real_escape_string($sub)."', '"
      .php2MySqlTime(js2PhpTime($st))."', '"
      .php2MySqlTime(js2PhpTime($et))."', '"
      .mysql_real_escape_string($ade)."', '"
      ...
      .mysql_real_escape_string($ta)."', '"
			...
			.mysql_real_escape_string($medio).")";
      //.mysql_real_escape_string($color)."' )";
			mysql_query ("SET NAMES 'utf8'");
      mysql_set_charset('utf8');
    //echo($sql);
