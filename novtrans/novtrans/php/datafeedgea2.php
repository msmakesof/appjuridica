...
function listCalendarByRange($sd, $ed, $resp){
  ...
  try{
    ...
    $db->getConnection();	
		$paryy = $resp;
	  $condi = " and responsable IN ('$paryy')";
	  $condi2 = " and dirigido_a IN ('$paryy') ";
...
join procesos dirigido_hacia on dirigido_hacia.abr2 = jqcalendar.dirigido_a  and dirigido_hacia.estado = 'A'
where starttime between  '".php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."'
and jqcalendar.tipo_auditoria > 0 and capacitador in ('$paryy') and jqcalendar.tipolista = 2 AND jqcalendar.estado <> 'I' 
UNION
select distinct jqcalendar.*, procesos.nombre_res, tipo_lista.nombre_tl, dirigido_hacia.proceson nomdirigido
from jqcalendar 
join listachqxtipoaudit on listachqxtipoaudit.tipo_auditoria = jqcalendar.tipo_auditoria 
...
join procesos dirigido_hacia on dirigido_hacia.abr2 = jqcalendar.dirigido_a  and dirigido_hacia.estado = 'A'
where starttime between  '".php2MySqlTime($sd)."' and  '". php2MySqlTime($ed)."' 
and jqcalendar.tipo_auditoria > 0 $condi2 and jqcalendar.tipolista = 3 AND jqcalendar.estado <> 'I' 
UNION
select distinct jqcalendar.*, procesos.nombre_res, tipo_lista.nombre_tl, dirigido_hacia.proceson nomdirigido
from jqcalendar 
join listachqxtipoaudit on listachqxtipoaudit.tipo_auditoria = jqcalendar.tipo_auditoria 
...
join procesos dirigido_hacia on dirigido_hacia.abr2 = jqcalendar.dirigido_a  and dirigido_hacia.estado = 'A'
where starttime between  '".php2MySqlTime($sd)."' and  '". php2MySqlTime($ed)."' 
and jqcalendar.tipo_auditoria > 0 and responsable IN ('$paryy') and dirigido_a in ('$paryy') and jqcalendar.tipolista = 3 AND jqcalendar.estado <> 'I' order by StartTime, responsable ";
    ...
    while ($row = mysql_fetch_object($handle)) {
      ...
      $ret['events'][] = array(
        $row->Id,			
				//$imgg." - ".$row->responsable." - ".$row->actividad.'  ',
				$imgg." - ".$row->actividad.'  ',
...
function listCalendar($day, $type, $resp){
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $ta, $au, $lc, $color, $observacion, $actividad, $tzz, $seguimiento){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
