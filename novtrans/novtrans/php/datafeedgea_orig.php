...
function listCalendarByRange($sd, $ed, $resp){
  ...
  try{
    ...
    mysql_set_charset('utf8');
		...
		$sql = "select distinct jqcalendar.*, procesos.nombre_res, tipo_lista.nombre_tl, dirigido_hacia.proceson nomdirigido from jqcalendar join listachqxtipoaudit on listachqxtipoaudit.tipo_auditoria = jqcalendar.tipo_auditoria and listachqxtipoaudit.id_auditoria = jqcalendar.auditoria and listachqxtipoaudit.id_listacheq = jqcalendar.listachq and listachqxtipoaudit.idproceso = jqcalendar.tipolista join procesos on procesos.abr2 = jqcalendar.responsable left join tipo_lista on tipo_lista.id = jqcalendar.tipolista join procesos dirigido_hacia on dirigido_hacia.abr2 = jqcalendar.dirigido_a where starttime between '".php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."' and jqcalendar.tipo_auditoria > 0 $condi  union select distinct jqcalendar.*, procesos.nombre_res, tipo_lista.nombre_tl, dirigido_hacia.proceson nomdirigido 
from jqcalendar 
...
left join procesos on procesos.abr2 = jqcalendar.responsable and procesos.estado ='A'
    ...
    while ($row = mysql_fetch_object($handle)) {
      ...
      //echo $row->StartTime;
			...
			if($row->tipolista == 3)
			{
					...
					//$txt_dirigido_a = " - Dirigido a: ".$fx_dirigido_a."   "; 
			}
