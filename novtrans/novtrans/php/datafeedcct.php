...
function listCalendarByRange($sd, $ed){
  ...
  try{
    ...
    mysql_set_charset('utf8');
		...
		//$condi = " and responsable='COORDINADOR DE CONTROL DE CUENTAS'";
		$condi = " and responsable ='CCT'";
		
		//$sql = "select jqcalendar.* from jqcalendar join tipo_auditoria on tipo_auditoria.cod_ti_audit = jqcalendar.tipo_auditoria join auditoria on auditoria.id = jqcalendar.auditoria join listachqxtipoaudit on listachqxtipoaudit.id_listacheq = jqcalendar.listachq where starttime between '".php2MySqlTime($sd)."' and '". php2MySqlTime($ed)."' and jqcalendar.tipo_auditoria > 0 $condi;";
		
		$sql = "select jqcalendar.*, procesos.nombre_res, tipo_lista.nombre_tl from jqcalendar join tipo_auditoria on tipo_auditoria.cod_ti_audit = jqcalendar.tipo_auditoria join auditoria on auditoria.id = jqcalendar.auditoria join listachqxtipoaudit on listachqxtipoaudit.id_listacheq = jqcalendar.listachq join procesos on procesos.abr2 = jqcalendar.responsable join tipo_lista on tipo_lista.id = jqcalendar.tipolista where starttime between '"		
