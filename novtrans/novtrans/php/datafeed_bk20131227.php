...
function listCalendarByRange($sd, $ed){
  ...
  //$ret["end"] = php2JsTime($ed);
	$ret["fecha"] = php2JsTime($sd);
  $ret["fecha"] = php2JsTime($ed);
  $ret['error'] = null;
  try{
    $db = new DBConnection();
    $db->getConnection();
    //$sql = "select * from jqcalendar where starttime between '"
		// $sql = "select jqcalendar.*, novedades.nombre from jqcalendar JOIN novedades ON novedades.id = jqcalendar.tiponovedad where starttime between '"
		
		// $sql = "SELECT plan_activs.*, cronograma.requisito, pregunta FROM crono_activs JOIN cronograma ON cronograma.itemlc = crono_activs.item WHERE starttime between '"
		
		 $sql = "SELECT plan_activs.* FROM plan_activs WHERE fecha between '"
...
function updateDetailedCalendar($id, $st, $et, $sub, $ade, $dscr, $loc, $color, $tiponovedad, $tz){
  $ret = array();
  try{
    $db = new DBConnection();
    $db->getConnection();
    $sql = "update jqcalendar set"
      ...
      . " color='" . mysql_real_escape_string($color) . "' "
			. " tiponovedad='" . mysql_real_escape_string($tiponovedad) . "' "
      . "where id=" . $id;
    //echo $sql;
		if(mysql_query($sql)==false){
      $ret['IsSuccess'] = false;
    ...
    case "adddetails":
        ...
        if(isset($_GET["id"])){
            $ret = updateDetailedCalendar($_GET["id"], $st, $et, 
                ...
                $_POST["Location"], $_POST["tiponovedad"], $_POST["colorvalue"], $_POST["timezone"]);
        }else{
