...
mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_tabla = "SELECT IdMateria, NombreMateria, Estado, Nombre_Estado FROM materia JOIN gen_estado ON gen_estado.Id_Estado = materia.Estado WHERE IdMateria = $idTabla ;" ;
...
$totalRows_rs_tabla = mysqli_num_rows($rs_tabla);
	...
	$idtabla = $row_rs_tabla["IdMateria"];
    $Nombre = trim($row_rs_tabla['NombreMateria']);
                ...
                if (e) {
                    ...
                    .done(function( dataX, textStatus, jqXHR ){                       
                        ...
                        {                    
                             $("#mensaje").hide();
                             $("#form_validation").show();
                             $("#msj").html('<div class="alert alert-danger"><span class="glyphicon-hand-right"></span><strong>  Atención: </strong> Registro ?> NO Borrado.</div>').fadeIn(3000);                    
                        }
                    })
                    .fail(function( jqXHR, textStatus, errorThrown ) {
                        //e.stopPropagation();
