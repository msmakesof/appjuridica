...
require_once('../../apis/general/submenu.php');
$Nombre = trim($msubmenu['men_submenu']['SME_Nombre']);
$estado = $msubmenu['men_submenu']['SME_Estado'];
$idtabla = $msubmenu['men_submenu']['SME_IdSubMenu'];
$idmenu = $msubmenu['men_submenu']['SME_IdMenu'];
$Icono = trim($msubmenu['men_submenu']['SME_Icono']);
$Link = $msubmenu['men_submenu']['SME_Link'];
?>
<!DOCTYPE html>
<html>
...
<body class="theme-indigo">
                        ...
                        <div class="body  table-responsive">
                            <form id="form_validation" method="POST">
								...
								<div class="form-float">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker show-tick" data-live-search="true" data-width="90%" name="menu" id="menu" style="z-index:5000 !important;" required>
                                            <?php 
                                                $idTabla = 0;
                                                require_once('../../apis/general/menu.php');                                                
                                                 for($i=0; $i<count($mmenu['men_menu']); $i++)
                                                {           
                                                    $MEN_IdMenu = $mmenu['men_menu'][$i]['MEN_IdMenu'];
                                                    $MEN_Nombre = $mmenu['men_menu'][$i]['MEN_Nombre'];
                                                    $MEN_Estado = $mmenu['men_menu'][$i]['MEN_Estado']; 
                                            ?>                                            
                                            <option value="<?php echo $MEN_IdMenu; ?>" <?php if ($MEN_IdMenu == $idmenu){ echo "selected";} else{ echo "";} ?>>
                                                <?php echo $MEN_Nombre ; ?>                                                
                                            </option>
                                            <?php                    
                                                } 
								...
								<div class="form-group form-float">
                                    ...
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="icono"id="icono" value="<?php echo $Icono ;?>" required>                                        
                                    </div>
                                </div>
	...
	{			
		...
		$("#grabar").on('click', function(e) {
			...
			$.ajax({
				data : {"menu": menu, "nombre": nombre, "estado": estado, "icono": icono, "link": link, "idtabla": idtabla},
				type: "POST",
				dataType: "html",
