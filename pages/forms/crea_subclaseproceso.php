...
{
  $parameters = "ExisteTabla=1&Nombre=$pnombre&ClaseProceso=$pclaseproceso&idtabla=0";
  $url = urlServicios."consultadetalle/consultadetalle_pro_subclaseproceso.php?".$parameters;
  //echo("<script>console.log('PHP existe: ".$url."');</script>");
  $ch = curl_init();
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['pro_subclaseproceso']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe una SubClase de Proceso registrado con el mismo Nombre.";
    }
    else
    {
      
      $parameters = "insert=insert&Nombre=$pnombre&ClaseProceso=$pclaseproceso&Estado=$pestado";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/consultadetalle_pro_subclaseproceso.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
      ...
      {    
          ...
          {          
            ...
            $m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
            $m = json_decode($m, true);
            $grabadoOK = $m['pro_subclaseproceso'];
            if(!$grabadoOK)
            {
