...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['pro_estadoactprocesal']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe un Estado Actuacion Procesal registrado con el mismo Nombre.";
    }
    else
    {
      
      $parameters = "insert=insert&Nombre=$pnombre&DiasHabiles=$pdias&Estado=$pestado";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/consultadetalle_pro_estadoactprocesal.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
      ...
      {    
          ...
          {          
            ...
            $m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
            $m = json_decode($m, true);
            $grabadoOK = $m['pro_estadoactprocesal'];
            if(!$grabadoOK)
            {
