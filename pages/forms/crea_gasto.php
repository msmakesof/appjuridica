...
{
    $pconcepto = trim(strtoupper($_POST['concepto']));
	$pconcepto = str_replace(' ','%20', $pconcepto);
}
...
{
  $parameters = "ExisteActPro=1&Proceso=$pproceso&FechaInicio=$pfechainicio&Concepto=$pconcepto&Gasto=$pgasto";
  $url = urlServicios."consultadetalle/pro_gastos.php?".$parameters;
  $ch = curl_init();
  ...
  {          
    ...
    else
    {
	  ...
	  $estadoActPro = "1";
      $parameters = "insert=insert&Proceso=$pproceso&Fechainicio=$pfechainicio&Concepto=$pconcepto&Usuario=$idUsuario&Gasto=$pgasto";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/pro_gastos.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
            ...
            else
            {
				...
				if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL				
				{
					...
					{          
						...
						$m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
						$m = json_decode($m, true);
						$MaxId = $m['pro_gasto']['MaxIdProceso'];
						$sigue = $sigue."*".$MaxId;
					}
				}
				else
