...
$picono ="";
if( isset($_POST['picono']) )
{
    $picono = trim($_POST['picono']);
    $picono = str_replace(' ','%20', $picono);
}

$porden ="";
if( isset($_POST['porden']) )
{
    $porden = trim($_POST['porden']);
    $porden = str_replace(' ','%20', $porden);
}

$plink ="";
if( isset($_POST['plink']) )
{
    $plink = trim($_POST['plink']);
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
  ...
  $url = urlServicios."consultadetalle/consultadetalle_men_menu.php?".$parameters;
  $ch = curl_init();
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['men_menu']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe un Tipo Usuario registrado con el mismo Nombre.";
    }
    else
    {
      
      $parameters = "insert=insert&Nombre=$pnombre&Icono=$picono&Orden=$porden&Link=$plink&Estado=$pestado";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/consultadetalle_men_menu.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
