...
{
  $parameters = "ExisteTabla=1&Nombre=$pnombre&Link=$plnk&idtabla=0";
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['gen_sitiofrecuente']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe un Sitio Frecuente Registrado para esta fecha.";
    }
    else
    {
      
      $parameters = "insert=insert&Nombre=$pnombre&Lnk=$plnk&Estado=$pestado";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/gen_sitiofrecuente.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
      $sigue       = ""; 
      //echo "<script>console.log('insert...'+$url)</script>" ;
      if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
      {    
          $ch = curl_init();
          curl_setopt($ch, CURLOPT_VERBOSE, true);
          ...
          {          
            ...
            $m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
            $m = json_decode($m, true);
            $grabadoOK = $m['gen_sitiofrecuente'];
            if(!$grabadoOK)
            {
