...
$query_rs_tdusuarios = "SELECT Id_Estado, Nombre_Estado FROM gen_estado ORDER BY Nombre_Estado ;" ;
$rs_tdusuarios = mysqli_query($cnn_kn, $query_rs_tdusuarios) or die(mysqli_error()."Err1.....$query_rs_tdusuarios<br>");
$row_rs_tdusuarios = mysqli_fetch_assoc($rs_tdusuarios);
$totalRows_rs_tdusuarios = mysqli_num_rows($rs_tdusuarios);


mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_sucusuarios = "SELECT IdNivel, NombreNivel, Estado FROM nivel ORDER BY NombreNivel ;" ;
$rs_sucusuarios = mysqli_query($cnn_kn, $query_rs_sucusuarios) or die(mysqli_error()."Err2.....$query_rs_sucusuarios<br>");
$row_rs_sucusuarios = mysqli_fetch_assoc($rs_sucusuarios);
$totalRows_rs_sucusuarios = mysqli_num_rows($rs_sucusuarios);

mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_Materia = "SELECT IdMateria, NombreMateria, Estado FROM materia WHERE Estado = 1 ORDER BY NombreMateria ;" ;
$rs_Materia = mysqli_query($cnn_kn, $query_rs_Materia) or die(mysqli_error()."Err3.....$query_rs_Materia<br>");
$row_rs_Materia = mysqli_fetch_assoc($rs_Materia);
$totalRows_rs_Materia = mysqli_num_rows($rs_Materia);

mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_tabla = "SELECT IdTemaxNivel, IdNivelTxN, IdTemaTxN, IdEstadoTxN FROM temasxnivel JOIN gen_estado ON gen_estado.Id_Estado = temasxnivel.IdEstadoTxN WHERE IdTemaxNivel = $idTabla ;" ;
//echo "qry_usu......$query_rs_tabla" ;
mysqli_set_charset($cnn_kn,"utf8");
$rs_tabla = mysqli_query($cnn_kn, $query_rs_tabla) or die(mysqli_error()."Err4.....$query_rs_tabla<br>");
$row_rs_tabla = mysqli_fetch_assoc($rs_tabla);
$totalRows_rs_tabla = mysqli_num_rows($rs_tabla);

	$idtabla = $row_rs_tabla["IdTemaxNivel"];
    $Nombre = trim($row_rs_tabla['IdNivelTxN']);
    $Materia = $row_rs_tabla['IdTemaTxN'];
    $Estado = $row_rs_tabla['IdEstadoTxN'];    
?>
<!DOCTYPE html>
<html>
...
<body class="theme-indigo">
                            ...
                            <form id="form_validation" method="POST">
                                ...
                                <div class="form-group form-float">
                                    ...
                                    <!--<div class="form-line">
                                     ...
                                     <select class="form-control show-tick" name="nivel" id="nivel" required>
                                            <option value="">-- Seleccione Nivel --</option>
                                            <?php                                                 
                                                do                                                 
                                                {           
                                                    $idNivel = $row_rs_sucusuarios["IdNivel"];
                                                    $NombreNivel = $row_rs_sucusuarios["NombreNivel"];
                                                    $EstadoNivel = $row_rs_sucusuarios["Estado"];
                                            ?>
                                            <option value="<?php echo $idNivel; ?>" <?php if ($idNivel == $Nombre){ echo "selected";} else{ echo "";} ?>>
                                                <?php echo $NombreNivel; ?>                                                
                                            </option>
                                            <?php                    
                                ...
                                <div class="form-group form-float">
                                    ...
                                    <div class="col-sm-4">
                                        <select class="selectpicker" data-show-subtext="true" data-live-search="true" data-style="btn-info" name="materia" id="materia" required>
                                            <?php                                                 
                                                do                                                 
                                                {           
                                                    $idMateria = $row_rs_Materia["IdMateria"];
                                                    $NombreMateria = $row_rs_Materia["NombreMateria"];
                                                    $EstadoMateria = $row_rs_Materia["Estado"];
                                            ?>
                                            <option value="<?php echo $idMateria; ?>" <?php if ($idMateria == $Materia){ echo "selected";} else{ echo "";} ?>>
                                                <?php echo $NombreMateria; ?>                                                
                                            </option>
                                            <?php                    
                                                } while($row_rs_Materia = mysqli_fetch_assoc($rs_Materia));
                                            ?>
                                        </select>
                                    </div>   
                                </div>                              
	...
	{			
		...
		$("#grabar").on('click', function(e) {
            ...
            if( nivel == "" || materia == "" || estado == undefined )
            {
            ...
            {  
    			...
    			$.ajax({
    				data : {"nombre": nivel, "sucursal": materia, "estado": estado, "idtabla": idtabla},                
    				type: "POST",				
    			...
    			.done(function( dataX, textStatus, jqXHR ){	
    				...
    				{					
                        $("#msj").html('<div class="alert alert-danger"><span class="glyphicon-hand-right"></span><strong>  Atención: </strong> Registro NO modificado.  Verificar si selecciono un Nivel o Topic ya asignados.</div>').fadeIn(3000);                     
    				}
    			})
