...
mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_tabla = "SELECT IdFuncionario, funIdFuncionario, IdCentroAcopio, ClaveFuncionario, EstadoFuncionario, NombresFuncionario, ApellidosFuncionario, IdIpsFuncionario, IdCargoFuncionario, EmailFuncionario, IdTipoDocumentofuncionario FROM funcionario WHERE IdFuncionario = $idTabla ;" ;
...
$totalRows_rs_tabla = mysqli_num_rows($rs_tabla);
	...
	$idtabla = $row_rs_tabla["IdFuncionario"];
    $TipoDocumento = trim($row_rs_tabla["IdTipoDocumentofuncionario"]);
    $NumeroDocumento = trim($row_rs_tabla['funIdFuncionario']);
    $NombreAlumno = trim($row_rs_tabla['NombresFuncionario']);
    $Apellido1 = trim($row_rs_tabla['ApellidosFuncionario']);    
    $EstadoEst = $row_rs_tabla['EstadoFuncionario'];
    $IdIpsFuncionario = $row_rs_tabla['IdIpsFuncionario'];
    $IdCargoFuncionario = $row_rs_tabla['IdCargoFuncionario'];
    $Clave = $row_rs_tabla['ClaveFuncionario'];    
    $Email = $row_rs_tabla['EmailFuncionario']; 
...
<body class="theme-indigo">
                    ...
                    <div class="card">
                        ...
                        <div class="body  table-responsive">
                            <form id="form_validation" method="POST">
                                ...
                                <div class="form-group form-float">
                                    <label class="form-label">IPS Funcionario</label>
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="ipsfuncionario" id="ipsfuncionario" value="<?php echo $IdIpsFuncionario ;?>" maxlength="50" required>
                                       <!-- -->
                                    </div>
                                </div>
                                <div class="form-group form-float">
                                ...
                                <div class="form-group form-float">
                                    ...
                                    <div class="form-line">
                                        <input type="text" class="form-control" name="cargo" id="cargo" value="<?php echo $IdCargoFuncionario ;?>" maxlength="13" required>
                                       <!---->
                                    </div>
                                </div>
	...
	{			
		...
		$("#grabar").on('click', function(e) {
            ...
            var clave = $("#clave").val();            
            var IdIpsFuncionario = $("#IdIpsFuncionario").val();
            var email = $("#email").val();
            var IdCargoFuncionario = $("#IdCargoFuncionario").val();
            var centroacopio = $("#centroacopio").val();
            
			var estado = $('input:radio[name=estado]:checked').val();
			var idtabla =  "<?php echo $idtabla; ?>";
            if( tipodocumento == "" || numerodocumento =="" || nombre == "" || apellido1 == "" || clave =="" || IdIpsFuncionario == "" || email == "" || IdCargoFuncionario == "" || estado == undefined || centroacopio =="")
            {
            ...
            {
    			$.ajax({
    				data : {"tipodocumento": tipodocumento, "numerodocumento": numerodocumento, "nombre": nombre, "apellido1": apellido1, "clave": clave, "IdIpsFuncionario": IdIpsFuncionario, "email": email, "IdCargoFuncionario": IdCargoFuncionario, "estado": estado,"idtabla": idtabla,  "centroacopio": centroacopio},                
    				type: "POST",				
            ...
            alertify.confirm( 'Desea borrar este registro?', function (e) {
                if (e) {
                    ...
                    $.ajax({
                        ...
                        type: "POST",
                        dataType: "html",
                        url : "../forms/borrar_funcionario.php",
                    })  
                    .done(function( dataX, textStatus, jqXHR ){                       
                        var respstr = dataX;        
                        if( respstr == "S" )
