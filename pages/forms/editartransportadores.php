...
mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_tabla = "SELECT NombreTransportador, ApellidosTransportador, IdTransportador, NitTransportador, DireccionTransportador, TelefonoTransportador, CelularTransportador, EmailTransportador, IdCentroAcopio, EstadoTransportador, IdTipoDocumentoTransportador FROM transportadores WHERE IdTransportador = $idTabla ;" ;
...
$totalRows_rs_tabla = mysqli_num_rows($rs_tabla);
	...
	$idtabla = $row_rs_tabla["IdTransportador"];
    $TipoDocumento = trim($row_rs_tabla["IdTipoDocumentoTransportador"]);
    $NumeroDocumento = trim($row_rs_tabla['NitTransportador']);
    $NombreAlumno = trim($row_rs_tabla['NombreTransportador']);
    $Apellido1 = trim($row_rs_tabla['ApellidosTransportador']);    
    $EstadoEst = $row_rs_tabla['EstadoTransportador'];
    $direccion = $row_rs_tabla['DireccionTransportador'];
    $celular = $row_rs_tabla['CelularTransportador'];
    $telefono = $row_rs_tabla['TelefonoTransportador'];    
    $Email = $row_rs_tabla['EmailTransportador']; 
...
<body class="theme-indigo">
	...
	{			
		...
		$("#grabar").on('click', function(e) {
            ...
            if( tipodocumento == "" || numerodocumento =="" || nombre == "" || apellido1 == "" || telefono =="" || direccion == "" || email == "" || celular == "" || estado == undefined || centroacopio =="")
            {
            ...
            {
    			$.ajax({
    				data : {"tipodocumento": tipodocumento, "numerodocumento": numerodocumento, "nombre": nombre, "apellido1": apellido1, "telefono": telefono, "direccion": direccion, "email": email, "celular": celular, "estado": estado,"idtabla": idtabla,  "centroacopio": centroacopio},                
    				type: "POST",				
