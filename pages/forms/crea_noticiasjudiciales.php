...
{
  $parameters = "ExisteTabla=1&Nombre=$pnombre&Texto=$ptexto&idtabla=0";
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['gen_noticiasjudiciales']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe una Noticia Judicial Registrado para esta fecha.";
    }
    else
    {
      ...
      {    
          ...
          {          
            ...
            $m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
            $m = json_decode($m, true);
            $grabadoOK = $m['gen_noticiasjudiciales'];
            if(!$grabadoOK)
            {
