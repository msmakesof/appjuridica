...
{
    $pdireccion = trim($_POST['pdireccion']);
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
  ...
  {          
    ...
    //print_r ($m);
    $existe = $m['gen_edificio']['existe'];
    if($existe > 0)
    {
      $sigue = "E-Existe un Edificio registrado con el mismo Nombre.";
    }
    else
    {
      
      $parameters = "insert=insert&Nombre=$pnombre&Direccion=$pdireccion&Estado=$pestado";
      $soportecURL = "S";
      $url         = urlServicios."consultadetalle/consultadetalle_gen_edificio.php?".$parameters;
      $existe      = "";
      $usulocal    = "";
      ...
      {    
          ...
          {          
            ...
            $m = preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
            $m = json_decode($m, true);
            $grabadoOK = $m['gen_edificio'];
            if(!$grabadoOK)
            {
              $sigue = "N-Registro NO ha sido grabado.". $curl_errno.' '.$curl_msj;
            }
            else
            {
