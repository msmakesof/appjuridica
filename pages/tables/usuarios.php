...
mysqli_select_db($cnn_kn, $database_cnn_kn);
$query_rs_tipo_tabla = "SELECT concat_WS(' ',Nombre_usuario, Apellido1_usuario) AS NombreUsuario, IdUsuario, Email_usuario, NombreSucursal, CASE EstadoUsuario WHEN 'M' THEN 'Activo' ELSE 'Inactivo' END EstadoUsuario FROM gen_usuarios LEFT JOIN sucursal ON sucursal.IdSucursal = gen_usuarios.IdCiudad_usuario WHERE Nombre_usuario <> '' AND Apellido1_usuario <> '' ORDER BY NombreUsuario ;";
...
do{
	...
	$idTabla = $row_rs_tipo_tabla['IdUsuario'];    
    $Email = trim($row_rs_tipo_tabla['Email_usuario']);
