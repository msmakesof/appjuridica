...
$txtnroproceso = "";
$condinroproceso = "";
$nroproceso = 0; //$_POST['proceso'];
$cadena = strlen($nroproceso);
if( $cadena <= 22 )
{
	$txtnroproceso = "<div style='float: right; margin-right:75px; border-radius: 30px; -moz-border-radius: 30px; -webkit-border-radius:30px;'><span class='label label-danger'>Actuaciones Procesales sin Nro. de Radicado o Nro. de Proceso. </span></div>";
}
else
{
            ...
            <div class="row clearfix">
                <div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
                    <div class="card">
                        <div class="header">
                            <h2>
                                Opciones para Exportar  <?php echo $txtnroproceso .' -  Pendiente'  ?>
                            </h2>
...
if($sesionUsuario != 4 )  // SA  => Muestra todos los clientes.
{
	...
	{
		$condi = "&IdTipouser=". $_SESSION['IdUsuario']; 
	}
	else // 1.Admin => Mostrarà todos los clientes de la empresa a la cual pertenece el usuario
	{
		$condi = "&IdTipouser=". $_SESSION['IdEmpresa']; 
	}
}

		
	$soportecURL = "S";
	$url         = urlServicios."consultadetalle/consultadetalle_pro_actuacionprocesal.php?IdMostrar=0&E=1&Proceso=".$midtabla.$condi ;
	$existe      = "";
	$usulocal    = "";
	$siguex      = "";
...
function nuevo(id)
{
	$.post('../forms/actprocesalcrea.php', { 'id': id }, function (result) {
        WinId = window.open('','_self');
        WinId.document.open();
        WinId.document.write(result);
        WinId.document.close();
