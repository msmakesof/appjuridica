...
require_once('../../Connections/DataConex.php');
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_pro_origenactprocesal.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
    ...
    $morigenactprocesal =  preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
    $morigenactprocesal = json_decode($morigenactprocesal, true);
    //echo("<script>console.log('PHP: ".print_r($morigenactprocesal)."');</script>");
    //echo("<script>console.log('PHP: ".count($m['pro_origenactprocesal'])."');</script>");
...
if($soportecURL == "N")
{
    ...
    $morigenactprocesal = json_decode($resultado, true);	        
} 

if( $morigenactprocesal['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($morigenactprocesal['pro_origenactprocesal']); $i++)
    {
        $NombreTabla = trim($morigenactprocesal['pro_origenactprocesal'][$i]['OAP_Nombre']);        
        $archivo = $NombreTabla.".php";
        $idTabla = $morigenactprocesal['pro_origenactprocesal'][$i]['OAP_IdOrigen'];
        $estadoTabla = trim($morigenactprocesal['pro_origenactprocesal'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
