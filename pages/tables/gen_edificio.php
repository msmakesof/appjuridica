...
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_gen_edificio.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
    ...
    $medificio =  preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
    $medificio = json_decode($medificio, true);
    //echo("<script>console.log('PHP: ".print_r($medificio)."');</script>");
    //echo("<script>console.log('PHP: ".count($m['gen_edificio'])."');</script>");
...
if($soportecURL == "N")
{
    ...
    $medificio = json_decode($resultado, true);	        
} 

if( $medificio['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($medificio['gen_edificio']); $i++)
    {
        $NombreTabla = trim($medificio['gen_edificio'][$i]['EDI_Nombre']);
		$Direccion = trim($medificio['gen_edificio'][$i]['EDI_Direccion']);		
        $archivo = $NombreTabla.".php";
        $idTabla = $medificio['gen_edificio'][$i]['EDI_IdEdificio'];
        $estadoTabla = trim($medificio['gen_edificio'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
