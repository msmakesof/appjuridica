...
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_pro_claseproceso.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
    ...
    $mclaseproceso =  preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
    $mclaseproceso = json_decode($mclaseproceso, true);
    //echo("<script>console.log('PHP: ".print_r($mclaseproceso)."');</script>");
    //echo("<script>console.log('PHP: ".count($m['pro_claseproceso'])."');</script>");
...
if($soportecURL == "N")
{
    ...
    $mclaseproceso = json_decode($resultado, true);	        
} 

if( $mclaseproceso['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($mclaseproceso['pro_claseproceso']); $i++)
    {
        $NombreTabla = trim($mclaseproceso['pro_claseproceso'][$i]['CPR_Nombre']);        
        $archivo = $NombreTabla.".php";
        $dias = $mclaseproceso['pro_claseproceso'][$i]['CPR_Dias'];
        $idTabla = $mclaseproceso['pro_claseproceso'][$i]['CPR_IdClaseProceso'];
        $estadoTabla = trim($mclaseproceso['pro_claseproceso'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
