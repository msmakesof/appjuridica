        ...
        <div class="container-fluid">
            <div class="block-header">
                <div class="col-lg-6 col-md-6 col-sm-6 col-xs-6">
                    <h2>
                        INFORMACION: Relación de <?php echo strtoupper($nombre_lnk); ?> &nbsp;
...
$IdTipouser = $_SESSION["TipoUsuario"];
	...
	$soportecURL = "S";
	$url         = urlServicios."consultadetalle/pro_gastos.php?IdMostrar=0&E=1&Proceso=".$midtabla."&IdTipouser=".$IdTipouser ;
	$existe      = "";
	$usulocal    = "";
	$siguex      = "";
	...
	if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
	{
		...
		$mgasto =  preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
		$mgasto = json_decode($mgasto, true);
		//echo("<script>console.log('PHP: ".print_r($mgasto)."');</script>");
		//echo("<script>console.log('PHP: ".count($m['pro_gasto'])."');</script>");
	...
	if($soportecURL == "N")
	{
		...
		$mgasto = json_decode($resultado, true);	        
	} 
	
	if( $mgasto['estado'] != 2)
	{
		$nombre_Tabla = "";
		$contar = count($mgasto['pro_gasto']);
		for($i=0; $i < $contar; $i++)
		{
			$idTablaap = $mgasto['pro_gasto'][$i]['GAS_IdGasto'];
			$idProceso = $mgasto['pro_gasto'][$i]['GAS_IdProceso'];			
			$FechaGestion = $mgasto['pro_gasto'][$i]['GAS_FechaGestionGasto'];			
			$Concepto = trim(strtoupper($mgasto['pro_gasto'][$i]['GAS_Concepto']));
			$Gasto = trim($mgasto['pro_gasto'][$i]['GAS_Valor']);
			$TotalGastos += $Gasto;
?>
        <tr>            
			<td href="javascript:void(0);" style="cursor: pointer;" onclick='actproupdg("<?php echo $idTablaap; ?>","<?php echo $midtabla; ?>", "<?php echo $idProceso; ?>")' ><?php echo $idTablaap; ?> - <?php echo $FechaGestion; ?></td>
...
function nuevo(id)
{
	$.post('../forms/actprocreagasto.php', { 'id': id }, function (result) {
        WinId = window.open('','_self');
        WinId.document.open();
        WinId.document.write(result);
        WinId.document.close();
...
function actproupdg(idactprocesal, idproceso, nroproceso)
{	
	$.post('../forms/actproupdgasto.php', { 'idactprocesal': idactprocesal,'idproceso': idproceso, "nroproceso": nroproceso }, function (result) {
        WinId = window.open('','_self');
        WinId.document.open();
        WinId.document.write(result);
        WinId.document.close();
