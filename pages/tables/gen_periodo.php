...
require_once('../../Connections/DataConex.php');
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_gen_periodo.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
    ...
    //echo("<script>console.log('PHP: ".print_r($mperiodo)."');</script>");
    //echo("<script>console.log('PHP: ".count($m['gen_periodo'])."');</script>");
...
if($soportecURL == "N")
{
    ...
    $mperiodo = json_decode($resultado, true);	        
} 

if( $mperiodo['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($mperiodo['gen_periodo']); $i++)
    {
        $NombreTabla = trim($mperiodo['gen_periodo'][$i]['PER_Nombre']);
		$Abreviatura = trim($mperiodo['gen_periodo'][$i]['PER_Abreviatura']);
        $archivo = $NombreTabla.".php";
        $idTabla = $mperiodo['gen_periodo'][$i]['PER_IdPeriodo'];
        $Valor = $mperiodo['gen_periodo'][$i]['PER_Valor'];
        $estadoTabla = trim($mperiodo['gen_periodo'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
