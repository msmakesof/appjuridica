...
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_pro_estadoproceso.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if(function_exists('curl_init')) // Comprobamos si hay soporte para cURL
{
    ...
    $mestadoproceso =  preg_replace('/[\x00-\x1F\x80-\xFF]/', '', $resultado);    
    $mestadoproceso = json_decode($mestadoproceso, true);
    //echo("<script>console.log('PHP: ".print_r($mestadoproceso)."');</script>");
    //echo("<script>console.log('PHP: ".count($m['pro_estadoproceso'])."');</script>");
...
if($soportecURL == "N")
{
    ...
    $mestadoproceso = json_decode($resultado, true);	        
} 

if( $mestadoproceso['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($mestadoproceso['pro_estadoproceso']); $i++)
    {
        $NombreTabla = trim($mestadoproceso['pro_estadoproceso'][$i]['EPR_Nombre']);        
        $archivo = $NombreTabla.".php";
        $idTabla = $mestadoproceso['pro_estadoproceso'][$i]['EPR_IdEstado'];
        $estadoTabla = trim($mestadoproceso['pro_estadoproceso'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
