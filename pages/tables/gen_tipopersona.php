...
$soportecURL = "S";
$url         = urlServicios."consultadetalle/consultadetalle_gen_tipopersona.php?IdMostrar=0";
$existe      = "";
$usulocal    = "";
$siguex      = "";
...
if( $mtipopersona['estado'] < 2)
{
    $nombre_Tabla="";
    for($i=0; $i<count($mtipopersona['gen_tipopersona']); $i++)
    {
        $NombreTabla = trim($mtipopersona['gen_tipopersona'][$i]['TPE_Nombre']);        
        $archivo = $NombreTabla.".php";
        $idTabla = $mtipopersona['gen_tipopersona'][$i]['TPE_IdTipoPersona'];
        $estadoTabla = trim($mtipopersona['gen_tipopersona'][$i]['EstadoTabla']);
    ?>
        <tr>
            <td>
