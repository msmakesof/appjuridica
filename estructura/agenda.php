...
class AGENDA
{
	...
	/**
     * Verifica si existe el Otra Agenda igual
     *
     * @param $IdUsuario identificador de Evento
     * @return bool Respuesta de la consulta
     */
    public static function buscadoble($From, $To, $TipoUsuario, $Responsable, $Proceso, $Tipo, $TipoActividad)
    {
        $consulta = "SELECT count(". $GLOBALS['Llave']. ") existe FROM ".$GLOBALS['TABLA'].
        " WHERE start = ? AND end = ? AND IdTipoUsuario = ? AND IdAsignado = ? AND IdProceso = ? AND class = ? AND IdTipoActividad = ? ;";
		
		//echo $consulta. " // $From, $To, $TipoUsuario, $Responsable, $Proceso, $Tipo, $TipoActividad";

        try {
            // Preparar sentencia
            $comando = Database::getInstance()->getDb()->prepare($consulta);
            // Ejecutar sentencia preparada
            $comando->execute(array($From, $To, $TipoUsuario, $Responsable, $Proceso, $Tipo, $TipoActividad));
            // Capturar primera fila del resultado
            $row = $comando->fetch(PDO::FETCH_ASSOC);
            return $row;
