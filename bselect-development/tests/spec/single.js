(function( $ ) {
    ...
    test( "markup structure", 5, function() {
        ...
        strictEqual(
            this.bselect.find( ".bselect-option" ).length, 3,
            "has the same number of items than the original select"
        );
        strictEqual(
            this.bselect.find( ".bselect-option[data-value='']" ).length, 0,
            "shouldn't have items with empty values"
        );
        ...
        ok(
            ...
            "should show a message telling that no options exist (issue #7)"
        );
    ...
    test( "optgroup support", function() {
        ...
        var bselect = select.bselect( "element" );
        var optgroup = bselect.find( ".bselect-option-group" );

        strictEqual( optgroup.length, 1, "has optgroup items in the dropdown" );
        strictEqual(
            optgroup.text(), "1",
            "the optgroup item has the same text as the original label"
        );
        strictEqual(
            bselect.find( ".bselect-option.grouped" ).length, 2,
            "the grouped items are differentiated with the .grouped class"
        );
